...
| Titres UI (cartes, panneaux, modales) | **Schibsted Grotesk** (grotesk éditoriale open-source) / Inter | `tokens.font.display` | 600-700 |
...
**Titres UI en Schibsted Grotesk** (2026-07, `tokens.font.display`) : le DM Sans
précédent tombait exactement dans le combo « Inter/DM Sans » que l'audit design pointe
comme générique. On le remplace par **Schibsted Grotesk**, une grotesk éditoriale
open-source sélectionnée sur *Free Faces* (`freefaces.gallery`) : sobre et rigoureuse
pour du médical, mais avec un caractère de presse qui casse la neutralité par défaut.
Réservée aux titres UI (cartes, panneaux, modales) ; le corps reste Inter, les titres
de page restent Source Serif 4. Changement 100 % réversible : un token + un lien Google
Fonts (`app/+html.tsx`).
...
- **`HeroBackdrop`** (`src/ui/HeroBackdrop.tsx` + `.web.tsx`) : grille millimétrée,
  source de lumière bleue **unique** qui « respire » lentement (cycle 14 s, keyframes
  `medinfo-hero-breathe` / classe `.medinfo-hero-glow` — profondeur ambiante d'esprit
  *Supahero*, jamais d'orbes multiples), tracé ECG en battement lent (cycle 9 s,
  keyframes `medinfo-ecg-draw`) et **grain filmique** en couche haute (`GrainOverlay`,
  cf. ci-dessous) — motifs métier, pas décor générique. Respiration et ECG coupés sous
  reduced-motion ; le grain est statique.
- **`GrainOverlay`** (`src/ui/GrainOverlay.web.tsx` + no-op natif, module pur
  `src/ui/grain.ts`) : grain « grainy gradient » d'esprit *Grainient* (`grainient.supply`)
  — un bruit SVG `feTurbulence` désaturé posé en `mix-blend-mode: overlay` sur les fonds
  bleu nuit (hero, dashboard, footer). Donne une matière au dégradé sans ajouter d'orbe
  de glow (banni par l'audit). Statique, décoratif (`aria-hidden`), web-first.
...
- `src/ui/HeroBackdrop.tsx` / `.web.tsx` — fond de hero (grille + lumière qui respire +
  ECG + grain, cf. §4).
- `src/ui/GrainOverlay.web.tsx` (+ no-op natif) / `src/ui/grain.ts` — grain filmique
  réutilisable sur les fonds bleu nuit (concept Grainient, cf. §4).
