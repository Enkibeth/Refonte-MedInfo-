...
Adaptation Refonte-MedInfo : pour le script "test", on prefere "test:unit" quand il existe.
La suite complete (`vitest run`) inclut les tests RLS qui exigent un Postgres local
(tests/rls/helpers/pgHarness.ts) ; les lancer ici bloquerait la porte sur un rouge d'infra
en session web/remote sans DB. Les tests RLS restent verifies en CI / quand une DB est dispo."""
...
if os.path.exists(pkg):
    ...
    for s in ("typecheck", "lint", "test"):
        ...
        target = s
        if s == "test" and "test:unit" in scripts:
            target = "test:unit"      # suite sans DB ; RLS exige un Postgres -> CI
        checks.append([runner, target] if runner == "yarn" else [runner, "run", target])
