...
| Résilience hors-ligne du chat (2026-06, **corrigée 2026-07**) | Actif | Tous (chat) | `/api/chat` (`consumeStream` + **`keepAlive`** `src/server/keepAlive.ts` + archivage serveur `onFinish`), reprise + bouton « Réessayer » dans `app/(chat)/chat.tsx` | Page suspendue pendant le streaming (iOS coupe le flux en quittant Safari) → la génération va au bout côté serveur et la réponse est archivée ; au retour, le client la récupère depuis l'historique. **Correctif 2026-07** : `consumeStream()` seul ne suffisait PAS en serverless (l'invocation est gelée dès que la réponse HTTP est avortée → génération jamais terminée, rien à retrouver) — `keepAlive()` prolonge l'invocation via le contexte de requête Vercel (`Symbol.for('@vercel/request-context')`, no-op ailleurs) ; fenêtre de reprise portée de **60 s à ~4 min** (une réponse evidence-first dépasse la minute), déclenchée aussi sur `pageshow` et **automatiquement à l'erreur** (plus besoin de cliquer « Réessayer », qui relançait un appel LLM pour rien) | ADR-0024 |
...
| Analyse des partiels (medoutils) | Actif (**v3, refonte 2026-07**) | Étudiants vérifiés | Page autonome `public/partiel.html` (servie statiquement par l'export web), embarquée en iframe par `app/(chat)/partiel.tsx` (RoleGate conservé ; l'en-tête vit côté natif, la page embarquée n'en a plus) ; moteur PUR dans le bloc `@partiel-logic` de la page, extrait et exécuté hors navigateur par `tests/unit/helpers/partielLogic.ts` → suites `tests/unit/partiel-parse.test.ts` (parsing) + `tests/unit/partiel-engine.test.ts` (stats/pondération/simulation/synthèse/suivi), **86 tests** dont le pipeline PDF de bout en bout (reconstruction → mêmes chiffres qu'en CSV) ; fumigation navigateur opt-in hors CI `scripts/dev/partiel-smoke.mjs` (CSV, .xlsx et **vrai PDF 2 pages** généré à la volée) | Import .xlsx/.xls/.csv/.pdf → **distribution RÉELLE** (histogramme des effectifs ; la loi normale n'est plus qu'une option décochée libellée « repère théorique » — la v2 traçait une gaussienne à la place des données), **z-scores** (comparer des épreuves de difficulté inégale), **coefficients** éditables et mémorisés localement (moyenne ET classement pondérés), **seuil de validation**, **base de classement** explicite (par défaut, seuls les étudiants notés sur TOUTES les épreuves incluses sont classés — un incomplet garde sa moyenne mais est affiché « hors classement » plutôt que comparé à des moyennes qui ne portent pas sur les mêmes épreuves ; bascule « tous les notés » possible, signalée comme un mélange), **synthèse déterministe** forces/faiblesses vs promo, **simulateur « et si »** (curseurs → moyenne/rang/centile à promo inchangée, l'ancienne moyenne étant retirée du classement) + note requise pour un objectif, **suggestions d'identifiant**, tri des tableaux, feuilles Excel multiples, exports CSV + PDF ; **CSV lus par un parseur maison testé** (`parseDelimited`/`decodeText`) — SheetJS lisait « 7,5 » comme 75 (virgule = séparateur de milliers) et corrompait les accents ; **chargement paresseux** des 4 librairies (~1,9 Mo évités au premier rendu ; un CSV n'en charge aucune) ; calcul 100 % client (aucune donnée envoyée, sans IA, aucune table, aucune migration) ; **suivi de progression local** : `localStorage` ne conserve QUE les résultats dérivés de l'étudiant lui-même (moyenne, rang, ses notes) + effectif de promo, **jamais les notes des autres** (données de tiers), enregistrement explicite et suppressible ; jamais de chiffre inventé (une absence n'est pas comptée 0, son coefficient sort du dénominateur ; identifiants en double et moyennes portant sur un nombre d'épreuves inégal signalés à l'écran) ; ancien `src/lib/classement.ts` supprimé | ADR-0019, **ADR-0035** |
...
| Module CV Builder (2026-06, **refonte v2 2026-08**) | Actif | Étudiants + professionnels vérifiés (et admins) | Page autonome `public/cv-builder.html` embarquée en iframe par `app/(chat)/cv-builder.tsx` (RoleGate + token de session par postMessage). **Moteur PUR** dans le bloc `@cv-engine` de la page (extrait et exécuté hors navigateur par `tests/unit/helpers/cvEngine.ts`, patron de `partiel.html`) : document **v2 VERSIONNÉ à sections libres** (aucune rubrique codée en dur ; `migrate()` idempotente depuis l'ancien format à rubriques figées — SEUL endroit du dépôt qui connaît la v1, le serveur ne migre rien), **apparence par défaut = le modèle de référence fourni par Hugo, coté au point près depuis les PDF** (liseré accent 30 pt au bord, bandeau 208,584 pt, colonne principale 218,584 → 565,28, corps 8 pt / interligne 8,8, titres de rubrique en ROMAIN 13,6 pt + filet gris 0,5 pt à 7,5 pt de la ligne de base, intitulés gras #333, dates grasses accent à droite, contacts espacés de 30 pt ; 7 tests « conformité au modèle » figent ces cotes), **7 polices au choix** (Helvetica par défaut — même famille métrique que le Liberation Sans des originaux, écart mesuré < 0,6 pt sur une ligne — /Times standard du PDF + Inter/Source Sans 3/Public Sans/EB Garamond/Lora, SIL OFL, sous-ensemblées WinAnsi dans `public/vendor/fonts/cv/`, chargées à la demande, embarquées dans le PDF pour ~7-9 ko par graisse RÉELLEMENT écrite ; police des titres dissociable de celle du corps), mesure du texte avec les métriques **identiques à celles inscrites par jsPDF** pour les 7 familles (test : écart < 0,001 pt → l'aperçu ne peut pas mentir sur la pagination), pagination explicite (entrée jamais coupée, titre jamais orphelin, saut forçable, débordement signalé), « Ajuster pour tenir en N pages » borné par des planchers de lisibilité, contraste WCAG AA, **export PDF VECTORIEL** (API texte de jsPDF) ; UI 3 volets (arborescence glisser-déposer, aperçu A4 éditable en ligne, inspecteur + onglet Thème), onglets sous 1024 px, undo/redo, bibliothèque locale multi-CV, import/export JSON, 3 modèles ; relecture IA `app/api/cv+api.ts` (`cv_review`) ; import `app/api/cv-import+api.ts` (`cv_import`, texte extrait client par pdf.js/mammoth) ; CRUD cloud `app/api/cv-docs+api.ts` ; validation + minimisation + normalisation d'import pures `src/cv/cvDocument.ts` ; table `cv_documents` (migrations `0029`+`0030`, **inchangées**) ; tests `tests/unit/cv-engine.test.ts` (49) + `tests/unit/cv-pdf.test.ts` (17, relit le PDF produit) + `tests/unit/cv-document.test.ts` ; fumigation navigateur opt-in `scripts/dev/cv-smoke.mjs` ; doc `docs/CV_BUILDER.md` | **Le PDF contient du VRAI TEXTE sélectionnable** — la v1 exportait une capture html2canvas, donc une IMAGE : les logiciels de tri (ATS) des CHU n'en extrayaient rien et la candidature était écartée sans être lue (html2canvas est retiré de la page). Avec une police embarquée, jsPDF écrit des NUMÉROS de glyphe (Identity-H) + une table `/ToUnicode` : c'est elle qui rend le CV relisible, et `tests/unit/helpers/pdfText.ts` la décode comme `pdftotext` (échec si un seul glyphe ne se retraduit pas) ; réseau coupé à l'export → repli sur une police standard plutôt qu'un export en échec. Création/édition/aperçu/export 100 % client, gratuits, illimités, sans mur d'inscription ; aucune ressource externe (librairies chargées à la demande) ; relecture IA + import = jamais de réécriture/invention — garde persona serveur étudiant/pro/admin (`resolveChatPersona`, refus 403 sinon), rate-limit (compteur étudiant) ; minimisation RGPD `sanitizeCvForAi` (jamais la photo, jamais les contacts) qui **PRÉSERVE LES INDEX** (un `fieldPath` `sections.2.entries.0.bullets.1` doit rester applicable : filtrer un vide appliquerait la correction au mauvais champ) ; photo jamais importée par l'IA ; sauvegarde cloud sur action EXPLICITE ; caractères hors WinAnsi (grec, CJK) impossibles à écrire dans les polices PDF standard → signalés avant l'export, jamais silencieusement remplacés ; RLS own-row stricte (test `tests/rls/cv-documents.test.ts`) ; un CV = donnée personnelle, jamais un dossier patient | ADR-0028, **ADR-0036** |
...
| `0043_profile_chat_country.sql` | Pays du chat persisté au profil : colonne `chat_country` (text, CHECK sur les codes connus, nullable) sur `profiles` — le choix ne vivait qu'en localStorage (perdu à chaque changement d'appareil) | Non (préférence d'affichage/orientation des sources — jamais une source de vérité serveur, body toujours borné par `coerceCountry`) | Own-row (lecture + écriture user, comme les infos perso 0017) ; hors verrou anti-élévation | Test `tests/rls/profile-personal-info.test.ts` (section 0043) ; appliquée via MCP le 2026-07-28 |
| `0044_chat_gpt56_luna.sql` | Latence + coût du chat (retour Hugo « c'est trop long ») : bascule des features `chat_researcher` et `chat_fast` de gpt-5-mini vers **gpt-5.6-luna** (tier le plus rapide/le moins cher de la famille GPT-5.6). Aucune table | Non | Service role only (hérite du verrou 0011) | `insert … on conflict do nothing` PUIS `update` ciblé (les lignes préexistent, 0041/0042) — l'update ne touche que les lignes encore sur `gpt-5-mini`, donc idempotent et respectueux d'un choix admin plus récent. La rédaction (`chat`, gpt-5.2) est VOLONTAIREMENT inchangée : basculer le rédacteur est un arbitrage de qualité clinique (1 clic dans le panel admin). ⚠️ La famille 5.6 remplace l'effort `minimal` par `none` → traduction au bord dans `openaiReasoningEffort` (`featureRuntime.ts`, pur, testé) |
...
| `0042_chat_fast_mode.sql` | Mode « Rapide » du chat : seed `ai_model_config` de la feature `chat_fast` (gpt-5-mini, openai, web_search OFF). Aucune table | Non | Service role only (hérite du verrou 0011) | Le mode Rapide enchaînait DEUX modèles (chercheur puis rédacteur) avec un budget de sortie de 1400 tokens raisonnement inclus : chemin le plus LENT et réponse parfois vide (« la réponse a peut-être été interrompue »). Il devient un appel unique sans outil ; `on conflict do nothing` (le POST admin fait un UPDATE) |
| `0041_chat_orchestrator_split.sql` | Split orchestrateur/rédacteur (audit 2026-07) : seed `ai_model_config` de la feature `chat_researcher` (gpt-5-mini, openai, web_search ON) — modèle bon marché qui rassemble un dossier de preuves ; la rédaction reste sur `chat` (gpt-5.2). ACTIF par défaut ; kill-switch `CHAT_ORCHESTRATOR_SPLIT=off` pour revenir au mono-modèle (décision Hugo 2026-07) | Non | Service role only (hérite du verrou 0011) | `on conflict do nothing` (le POST admin fait un UPDATE) ; phase 1 = `generateText`(mini)+outils dans `/api/chat`, phase 2 = `streamText`(gpt-5.2) sans outils depuis le dossier ; fail-open (recherche KO → repli mono-modèle) ; appliquée via MCP le 2026-07-23 |
...
|-------------|-----------|-------------------|----------|
| `chat_fast` | `/api/chat` (mode Rapide) | **gpt-5.6-luna** (web_search OFF ; migration `0044`) — UNE réponse directe, sans outil ni split ; le prompt du mode interdit toute source non vérifiée (migration `0042`) | Tous |
...
| `chat_researcher` | `/api/chat` (phase 1) | **gpt-5.6-luna** (web_search ON ; migration `0044`) | **Split orchestrateur/rédacteur** (ACTIF par défaut ; kill-switch `CHAT_ORCHESTRATOR_SPLIT=off`) : modèle bon marché qui porte la boucle d'outils et rassemble un dossier de preuves vérifié ; la rédaction clinique reste sur `chat` (gpt-5.2). ~-29 % de coût estimé. `src/ai/chat/split.ts` (pur, testé), migration `0041` ; **2026-08 : gpt-5-mini → gpt-5.6-luna** (tier le plus rapide et le moins cher d'OpenAI : 0,20 $/1 M entrée et 1,20 $/1 M sortie contre 0,25 $/2,00 $) — c'est cette phase qui porte la latence (~15-18 s par étape d'outil) et l'essentiel des tokens d'entrée ; la RÉDACTION clinique reste sur `chat` (gpt-5.2) |
