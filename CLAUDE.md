...
- **Hébergement : serveur Node autonome chez Hostinger (migration 2026-08, Vercel RETIRÉ)** :
  un processus unique `server/index.mjs` (→ `expo-server/adapter/http`, entrée hPanel
  `server.js`) sert `dist/client` ET toutes les routes `+api.ts`. `vercel.json`,
  `api/index.js`, `scripts/vercel/`, `@vercel/analytics` et `@vercel/speed-insights` ont été
  supprimés. Build = `npm run build`, démarrage = `npm start`. Runbook complet :
  `docs/09_DEPLOYMENT.md`. Pièges qui se manifestent en silence : (1) dans hPanel, le preset
  DOIT être **Other** avec un fichier d'entrée — le preset « React » déploierait un statique
  sans processus Node, donc sans aucune route API ; (2) les `EXPO_PUBLIC_*` sont inlinées AU
  BUILD côté client mais lues à l'EXÉCUTION côté routes API → les déclarer une fois dans
  hPanel (qui les injecte aux deux moments) et **redéployer** après changement ; (3) le proxy
  de l'hébergeur doit être sans buffering avec un timeout large, sinon le streaming du chat
  n'arrive qu'à la fin ; (4) `expo-server` doit être chargé par `require` (`createRequire`) —
  sa build ESM publiée est cassée (imports sans extension) ; (5) `keepAlive()` ne sert plus
  qu'à neutraliser un rejet détaché (le processus reste vivant, `consumeStream()` suffit) ;
  (6) le cron hebdo du blog passe par le cron système
  (`scripts/hostinger/weekly-blog-cron.sh`), plus par la plateforme. Hébergeur nommé dans les
  mentions légales : `src/deploy/hosting.ts` (LCEN art. 6-III). Runtime vérifié : **une seule
  dépendance** (`expo-server`), les routes API sont bundlées et autonomes. Fumigation :
  `npm run smoke:node` (nécessite un build).
