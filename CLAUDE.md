...
- Les illustrations du chat (marqueur `<!--IMG:…-->`) requièrent `GOOGLE_SEARCH_API_KEY` + `GOOGLE_SEARCH_ENGINE_ID` (Google Programmable Search Engine, Custom Search API) côté Vercel ; sans ces clés, la section de prompt n'est pas injectée et le chat reste 100% texte (fail-safe). Quota gratuit 100 recherches/jour — cache serveur 24h par requête dans `/api/image-search`.
