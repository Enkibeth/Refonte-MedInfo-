...
| `0045_chat_back_to_basics.sql` | Retour à la base du chat (ADR-0037) : `chat` → **gpt-5.6-luna** (`temperature` remise à NULL — l'API 5.6 refuse ce paramètre) et SUPPRESSION des lignes `chat_researcher`, `chat_fast`, `pubmed_agent` (+ leurs overrides de prompts). Aucune table | Non | Service role only (hérite du verrou 0011) | `ai_interactions` n'est PAS touchée : l'historique des coûts de ces features reste lisible (libellés « (retiré) » conservés dans le panel admin). Le test RLS de seed passe de 23 à 20 lignes |
...
|-------------|-----------|-------------------|----------|
| `chat` | `/api/chat` | **gpt-5.6-luna** (web_search ON ; migration `0045`) — UN SEUL appel LLM par réponse, aucune boucle d'outils (ADR-0037) | Tous — 3 chatbots (prompts `public`/`student`/`professional`) ; public → chat public seul, étudiant/pro vérifiés → les 3 |
