...
| Module CV Builder (2026-06) | Actif | Étudiants + professionnels vérifiés (et admins) | Page autonome `public/cv-builder.html` (éditeur structuré, aperçu A4 live, export PDF par impression vectorielle `window.print()`, relecture IA, historique cloud), embarquée en iframe par `app/(chat)/cv-builder.tsx` (RoleGate + token de session par postMessage) ; relecture IA `app/api/cv+api.ts` (feature `cv_review`, `generateObject` + schéma Zod) ; CRUD cloud `app/api/cv-docs+api.ts` ; validation + minimisation pures `src/cv/cvDocument.ts` (test `tests/unit/cv-document.test.ts`) ; table `cv_documents` (migration `0029`) | Création/édition/aperçu/export PDF 100% client et gratuits ; relecture IA = suggestions à valider, jamais de réécriture auto (le prompt n'invente aucun fait ni date) — garde persona serveur étudiant/pro/admin (`resolveChatPersona`, refus 403 sinon), rate-limit (compteur étudiant) ; minimisation RGPD `sanitizeCvForAi` (jamais la photo ni les coordonnées des référents par défaut) ; RLS own-row stricte (test `tests/rls/cv-documents.test.ts`) ; un CV = donnée personnelle, jamais un dossier patient | ADR-0028 |
