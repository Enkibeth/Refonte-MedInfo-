...
| Planificateur de révisions étudiant (2026-06) | Actif | Étudiants vérifiés (+ admins) | Écran natif `app/(chat)/revision.tsx` (RoleGate `revision`), moteur déterministe pur `src/features/revision/engine/` (`workload`/`riskScoring`/`planner`/`dates`), bornage `src/features/revision/plans.ts`, client `src/features/revision/api.ts`, UI `RevisionDashboard`/`PlanEditor`/`PlanHealthGauge`/`RevisionCalendar`/`BurnDownChart`/`AiBoostPanel`, route CRUD `app/api/revision+api.ts`, tables `revision_plans` + `revision_plan_items` (migration `0027`) | **Safe-box non-MDSW** : données PÉDAGOGIQUES uniquement (volumes pages/chapitres/QCM, dates, rythme déclaré, progression) — jamais symptôme/cas patient/diagnostic/CAT. Cœur 100 % déterministe (tout chiffre vient de l'utilisateur ou d'un calcul explicite) ; tâches du jour cochables + annulation + journalisation partielle ; visualisations semaine/mois/burn-down ; sauvegarde débouncée. RLS own-row stricte (test `tests/rls/revision-plans.test.ts`) ; route scopée au token. Base de référentiels (sans contenu copyrighté) **différée** (ADR-0027 « Suivi ») | ADR-0027 |
| Révisions — AI Boost (2026-06) | Actif | Étudiants vérifiés (+ admins) | Feature IA `revision_boost`, route `app/api/revision-boost+api.ts`, cœur pur `src/features/revision/boost.ts` (test `tests/unit/revision-boost.test.ts`), UI `AiBoostPanel.tsx`, prompt `revision_boost` (promptStore), seed `0028_revision_boost.sql` | L'IA **propose seulement** (l'utilisateur applique/ignore) ; vocabulaire d'actions **borné** (tampon/repos/plafond/révision espacée/priorité d'un bloc existant), parser **fail-closed** (anti-invention de volume/bloc/source) ; chiffres recalculés **serveur** (jamais le body) ; refus de toute sortie clinique (non-MDSW) ; persona serveur étudiant/admin + rate-limit ; disclosure AI Act | ADR-0027 |
...
| `0028_revision_boost.sql` | AI Boost du planificateur : seed `ai_model_config` de la feature `revision_boost` (claude-sonnet-4-6, anthropic). Aucune table | Non (suggestions d'organisation des révisions ; jamais de santé) | Service role only (hérite du verrou 0011) | Le POST admin fait un UPDATE, la ligne doit préexister (convention 0011) ; ADR-0027 |
...
| `revision_boost` | `/api/revision-boost` | claude-sonnet-4-6 | Étudiant (AI Boost du planificateur de révisions — suggestions bornées, l'utilisateur valide) |
