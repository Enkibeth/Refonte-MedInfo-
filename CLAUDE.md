...
| Historique des conversations + export PDF | Actif | Tous (chat) | `src/chat/history.ts` (CRUD client + `renameConversation`), `src/chat/serverHistory.ts` (archivage serveur ; `replaceLast` : une régénération REMPLACE la dernière réponse archivée au lieu de l'empiler — audit 2026-07), `src/ui/chat/HistoryPanel.tsx` (`ConversationList` réutilisée : modale mobile + colonne persistante desktop shell dans `chat.tsx`), `src/ui/chat/ChatbotSwitcher.tsx`, `src/chat/exportChatPdf.ts`, migration `0020_chat_history.sql` ; **toggle afficher/masquer la colonne desktop (2026-07)** : bouton `panelLeft` dans l'en-tête + repli dans le rail, préférence `localStorage` (`medinfo.chat.historyRail`) | RLS own-row stricte (`chat_conversations`/`chat_messages`), test `tests/rls/chat-history.test.ts` ; contenu potentiellement sensible ; depuis 2026-06 la réponse assistant est archivée par `/api/chat` (service role, propriété de la conversation vérifiée contre le user du token, jamais le body) | ADR-0024 |
| Pièces jointes du chat — documents & photos (2026-07) | Actif | Les 3 chatbots (comptes connectés ; web) | Bouton trombone du composer `app/(chat)/chat.tsx`, sélection + compression client `src/chat/pickAttachments.web.ts` (photos redimensionnées ≤ 1600 px / JPEG 0,85 au canvas ; PDF ≤ 3 Mo), validation serveur PURE `src/chat/attachments.ts` (`scanChatAttachments`, test `tests/unit/chat-attachments.test.ts`) branchée dans `app/api/chat+api.ts`, envoi en `file parts` AI SDK (`convertToModelMessages`), rendu des vignettes dans la bulle utilisateur | Le document/photo n'est JAMAIS stocké (ni archivé — `chat_messages` ne garde qu'une mention textuelle « (Pièce jointe : …) » — ni loggé), même doctrine que l'outil Analyse de document ; data URL exigée (anti-SSRF, aucun téléchargement serveur) ; types whitelistés (JPEG/PNG/WebP/GIF + PDF), plafonds de taille/nombre par message et par requête (pièces des tours passés comptées), refus serveur (401) de toute pièce jointe **anonyme** (l'essai invité reste texte seul) ; aucune nouvelle feature IA, aucune migration | ADR-0024 |
