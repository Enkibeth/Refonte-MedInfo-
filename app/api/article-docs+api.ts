/**
 * CRUD de l'historique cloud des articles (table `article_documents`, ADR-0031).
 *
 * Appelé par la page autonome (iframe `public/article.html`) avec le Bearer token de
 * session. On crée un client Supabase SCOPÉ AU TOKEN : la RLS own-row (migration 0033)
 * est la barrière réelle — un utilisateur ne lit/écrit/supprime QUE ses manuscrits.
 *
 *   GET    /api/article-docs            → liste (id, title, doc_type, updated_at)
 *   GET    /api/article-docs?id=<uuid>  → un article complet (document)
 *   POST   /api/article-docs            → upsert { id?, title, docType, document }
 *   DELETE /api/article-docs?id=<uuid>  → supprime
 *
 * Pas d'appel LLM ici (ce n'est pas une feature IA) : les aides vivent dans /api/article.
 * Un manuscrit est le travail scientifique de son auteur → RLS own-row stricte, jamais
 * le service_role.
 */
import { createClient, type SupabaseClient } from '@supabase/supabase-js';

import { getBearerToken } from '@/auth/serverIdentity';
import { coerceArticleId, sanitizeArticlePayload } from '@/article/articleDocument';
...
export async function GET(request: Request): Promise<Response> {
  ...
  const id = coerceArticleId(new URL(request.url).searchParams.get('id'));

  if (id) {
    const { data, error } = await auth.client
      .from('article_documents')
      .select('id, title, doc_type, document, created_at, updated_at')
      .eq('id', id)
      .maybeSingle();
...
export async function POST(request: Request): Promise<Response> {
  ...
  const sanitized = sanitizeArticlePayload(body);
  if (!sanitized.ok) return Response.json({ error: sanitized.error }, { status: 400 });
  const { title, docType, document } = sanitized.value;

  const id = coerceArticleId((body as { id?: unknown }).id);
  ...
  if (id) {
    const { data, error } = await auth.client
      .from('article_documents')
      .update({ title, doc_type: docType, document, updated_at: now })
      .eq('id', id)
  ...
  const { data, error } = await auth.client
    .from('article_documents')
    .insert({ user_id: auth.userId, title, doc_type: docType, document })
    .select('id, updated_at')
    .maybeSingle();
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  const { error } = await auth.client.from('article_documents').delete().eq('id', id);
